    ...
    public class TablasGenerales : System.Web.Services.WebService
    {
        ...
        public DataTable SP_ListaLineas_NegxCEO(string V_CODIGO, string V_UNI_OPE, string UserName)
        {
            try
            {
                ...
                dtResultados = (new GeneralSoapClient()).SP_ListaLineas_NegxCEO(V_CODIGO, V_UNI_OPE, UserName);
        ...
        [WebMethod(Description = "Lista Material")]
        public DataTable ListaMateriales(string V_NOMBRE, string UserName)
        {
            return (new GeneralSoapClient()).listamatxcodxdescrip(V_NOMBRE, UserName);
        }
        ...
        public DataTable ListaTipoBien(string UserName)
        {
            try
            {
                string cacheKey = $"ListaTipoBien"; // Combinar los filtros para crear una clave única para la caché
                MemoryCache cache = MemoryCache.Default;  // Obtener la instancia del MemoryCache
                                                          // Verificar si ya existe el resultado en caché
                if (cache.Contains(cacheKey))
                {
                ...
                dtResultados = (new GeneralSoapClient()).TipoBien(UserName);             // Si no está en caché, llamar al servicio para obtener los datos
                ...
                if (!string.IsNullOrEmpty(UserName))  // Almacenar el resultado en caché, si es un valor correcto
                {
                    cache.Add(cacheKey, dtResultados, policy);
                }
        ...
        [WebMethod(Description = "listalineas_simaperu30")]
        ...
        [WebMethod(Description = "listadecisión34")]
        public DataTable listadecisión34(string UserName)
        {
            return (new GeneralSoapClient().listadecisión34(UserName));
        }
        ...
        {
            return (new GeneralSoapClient()).Lista_PAIS_DPT_PROV_DIST(V_OPCION, V_VAR, UserName);
        }
